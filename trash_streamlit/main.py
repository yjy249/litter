...
from trash.detect import start_detect
import os
import argparse
    ...
    if source_index == 0:
        ...
        if uploaded_file is not None:
            ...
            with st.spinner(text='Resource Loading...'):
                ...
                picture = picture.save(f'trash_streamlit/trash/data/images/{uploaded_file.name}')
                opt.source = f'trash_streamlit/trash/data/images/{uploaded_file.name}'
        else:
            is_valid = False
    else:
        uploaded_file = st.sidebar.file_uploader("Upload image", type=['mp4'])
        if uploaded_file is not None:
            is_valid = True
            with st.spinner(text='Resource Loading...'):
                st.sidebar.video(uploaded_file)
                with open(os.path.join("trash_streamlit/trash/data", "videos", uploaded_file.name), "wb") as f:
                    f.write(uploaded_file.getbuffer())
                opt.source = f'trash_streamlit/trash/data/videos/{uploaded_file.name}'
        else:
            is_valid = False
